...
DEFAULT_OUTPUT = "output/output_semantic_headers_FAISS.xlsx"
DEFAULT_CORPUS = "corpus/semantic_with_table_headers"
...
def build_hybrid_retriever_from_corpus(
    ...
    cache_path = Path(corpus_path) / "embeddings_cache.pkl"
    cache_meta_path = Path(corpus_path) / "embeddings_cache_meta.json"
    expected_meta = {
    ...
    try:
        ...
        index_path = Path(corpus_path) / "faiss_index.idx"
    ...
    def retrieve(query: str):
        ...
        final_docs = []
        for doc_idx, doc, score in reranked:
...
def build_rag_context(retriever, query: str, max_chars: int = 10000) -> str:
    ...
    for i, doc in enumerate(retrieved, start=1):
        ...
        segment = f"[{i}] From {source}{score_text}\n{doc.page_content}\n"
...
def main():
    ...
    parser.add_argument("--corpus", default=DEFAULT_CORPUS, help="RAG knowledge base folder (contains .jsonl files)")
    ...
    parser.add_argument("--max_context_chars", type=int, default=10000, help="Maximum context characters to inject into prompt")
    ...
    for pos, q in df.loc[start_pos:, question_col].items():
        ...
        user_prompt = (
            "Question:\n"
            f"{q_text}\n\n"
            "Retrieved Context (LangChainRAG from Corpus):\n"
